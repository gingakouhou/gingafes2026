...
import type { News, Event } from "@/lib/microcms";
...
export default function HomeClient({ newsList, eventList }: HomeClientProps) {
  // YouTubeプレイヤーインスタンスの状態管理
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [player, setPlayer] = useState<any>(null);

  // YouTubeの onReady で呼ばれるコールバック
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handlePlayerReady = useCallback((p: any) => {
    setPlayer(p);
  }, []);

  // 入場ボタンクリック時 → 直接 playVideo() を呼び出す
